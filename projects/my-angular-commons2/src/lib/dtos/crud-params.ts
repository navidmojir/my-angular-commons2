...
export class CrudParams {
	...
	// constructor(private defaultErrorMessageHandler: DefaultErrorMessageHandler){
	// 	this.errorMessageHandler = this.defaultErrorMessageHandler;
	// }

    baseUrl: string = "http://localhost:8080/";
    resourceName: string = "resource";
	searchMethod: string = "/search";
    // private _operations: Operation[];
    fieldConfigs: FieldConfig[] = [];
    errorMessageHandler: IErrorMessageHandler = inject(DefaultErrorMessageHandler);
    rtl: boolean = true;
    labels: LabelConfigs = new LabelConfigs();
    totalPagesHeaderName: string = "X-TOTAL-COUNT";
